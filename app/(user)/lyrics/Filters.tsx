...
const Filters = ({ Params, reciters }: Props) => {
	...
	const filterOptions = [{ filterKey: 'type', placeholder: 'Type', options: LyricsTypes }];
	...
	return (
		<div className="pb-12">
			<div className="my-2 text-sm md:text-lg">Filters</div>
			<Carousel className="min-w-full">
				<CarouselContent className="pr-1 pl-2">
					<CarouselItem className="flex basis-[46%] pl-2 md:basis-[30%] lg:basis-[22%]">
						<MagicBox
							open={reciterState}
							...
							trigger={
								<Button
									...
									size="sm"
									className={cn(
										'w-full justify-between rounded-full py-1 capitalize',
						...
						>
									...
									<CommandGroup className="min-h-[150px]">
										{reciters.map((item) => (
											...
											>
												{item.name}
												<CheckIcon
													className={cn('ml-auto h-4 w-4', item.id === reciter.id ? 'opacity-100' : 'opacity-0')}
												/>
											</CommandItem>
										))}
									</CommandGroup>
							...
							<div className="sticky bottom-0 -mb-4 flex items-center justify-center gap-2 border-t p-2">
								<Button className="w-full" variant="outline" onClick={handleClearReciter}>
									Clear
								</Button>
								<ButtonWithLoading
					...
					{filterOptions.map((filterOption, index) => (
						<CarouselItem key={index} className="flex basis-[46%] pl-2 md:basis-[30%] lg:basis-[22%]">
							<MagicBox
								...
								trigger={
									<Button
										...
										className={cn(
											'w-full justify-between rounded-full py-1 capitalize',
											filters[filterOption.filterKey as FilterKey].id &&
												'bg-primary hover:bg-primary/80 text-background hover:text-background',
										)}
									>
										...
										<CommandGroup className="min-h-[150px]">
											{filterOption.options.map((item) => (
												<CommandItem
													...
													value={item.id}
													onSelect={() =>
														handleSelect(filterOption.filterKey as FilterKey, { id: item.id, name: item.name })
													}
												>
													{item.name}
													<CheckIcon
