...
import { arrayToString, convertLyricsArr, getRecitersLinks } from '@/lib/utils/lyrics';
...
export async function generateMetadata(props: Params) {
  ...
  const { title, reciter: { name: Reciter }, type } = data;
...
export default async function Slug(props: Params) {
  ...
  const { title, topics, writers, reciter: Reciters, reciter: { name: Reciter }, otherReciters, type, dop, updatedAt } = data;
  ...
  return (
    <article>
      ...
      />
      <GradientHeading className='my-2 text-2xl md:text-4xl font-extrabold' h={1}>{title} Lyrics - {Reciter}</GradientHeading>
      <p>Let&#8217;s Check Out The <strong>{title} Lyrics</strong>, Recited by <strong>{recitersList}</strong>. It is a New <Link className='a' href={`/lyrics?type=${type}`}>{capitalize(type)}</Link> of the
        Year {year}. {writers.length > 0 && `Its Beautiful Lyrics Are Written By ${writersNames}.`}
      </p>
      ...
      <div className='flex flex-col md:flex-row items-center justify-center gap-4 mx-auto my-10'>
        ...
        <div className='w-full md:w-[65%] px-2 mt-4 md:mt-0 relative'>
          <div className='flex items-center justify-start gap-x-2'>
            {/* {audioExists && <PlayBtn slug={slug} title={title} reciterName={recitersList} reciterSlug={reciter} />} */}

            <div className='mt-6'>
              <h2 className='text-sm opacity-90 my-1 text-primary'>Title</h2>
          ...
          <div className='grid grid-cols-2 gap-x-4 mt-5'>
            <div>
              <h2 className='text-sm opacity-80 my-1 text-primary'>Recited By</h2>
              <p className='my-1 text-xs'>{recitersLinks}</p>
            </div>
            <div>
              <h2 className='text-sm opacity-80 my-1 text-primary'>Written By</h2>
      ...
      <div className='my-10 grid grid-cols-1 md:grid-cols-7 lg:grid-cols-8 gap-2'>
        <div className='w-full my-4 relative md:col-span-5 lg:col-span-5'>
          <Interactions
            userId={userId}
            favorited={favorited}
            lyricsData={data}
          ...
          >
            ...
            <div id="urdu-lyrics"
              style={{ ...gulzar.style, textAlign: "center" }}
            >
              <LyricsContent
                paragraphs={urdu}
                title={title}
