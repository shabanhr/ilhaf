...
const CreateReportForm = ({ setOpen, selection }: Props) => {
	...
	return (
		<>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Textarea className="opacity-50" disabled={isSubmitting} {...field} readOnly />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Textarea disabled={isSubmitting} {...field} placeholder="Enter the correct lyrics!" />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
			...
			<ModalFooter>
				<ModalClose asChild>
					<Button variant="outline" color="danger" disabled={isSubmitting}>
						Close
					</Button>
				</ModalClose>
				<ButtonWithLoading className="w-full md:w-max" isLoading={isSubmitting} onClick={form.handleSubmit(onSubmit)}>
					Submit
				</ButtonWithLoading>
			</ModalFooter>
		</>
