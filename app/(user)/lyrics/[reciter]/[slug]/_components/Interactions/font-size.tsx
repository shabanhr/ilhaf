...
import { lyricsFontSizes } from './tab-triggers';
...
export const InteractionFontSize = () => {
	...
	const [fontSizeIndex, setFontSizeIndex] = useState(() => lyricsFontSizes.findIndex((size) => size.id === fontSizeId));

	const handleFontSizeChange = (value: number[]) => {
		const index = value[0];
		setFontSizeIndex(index);
		useLyricsInteractions.setState({ fontSizeId: lyricsFontSizes[index].id });
	};
