...
import { XSocialIcon } from '@/components/icons';
	...
	return (
		<>
			{selectionRef.current && selectionRef.current.length >= 10 && (
				<div
					className="bg-primary text-background absolute m-0 flex h-max w-max items-center justify-center rounded-xl px-2 py-1"
					style={{
						top: `${position.top}px`,
						left: `${position.left}px`,
					}}
				>
					<Button title="Share on X" variant="ghost" className="size-8 p-2" onClick={() => onShare()}>
						<XSocialIcon className="h-full w-full" />
					</Button>
					<Button
					...
					>
						<MessageCircleWarning className="h-full w-full" />
					</Button>
				</div>
			)}
			{userId && (
				<Modal open={open} onOpenChange={setOpen}>
					<ModalContent>
						...
						<ModalBody>{open && <CreateReportForm setOpen={setOpen} selection={selection} />}</ModalBody>
					</ModalContent>
				</Modal>
			)}
		</>
