...
const lyricsTypes = ["NOHA", "MANQABAT", "MANAJAT", "NAAT", "SALAM", "HAMAD", "DUA", "MARSIYA", "RAMZAN_KALAM", "SUFI_KALAM"] as const;
...
export const AddLyricsSchema = z.object({
    ...
    reciterId: z.string(),
    writers: z.array(z.string().min(1)).default([]),
    topics: z.array(z.string().min(1)).default([]),
    video: z.string(),
    otherReciters: z.array(z.string().min(1)).default([]),
    dop: z.date(),
...
export type ModelName = 'user' | 'lyrics' | 'topic' | 'reciter' | 'otherReciter' | 'writer' | 'account' | 'session' | 'verificationToken';
