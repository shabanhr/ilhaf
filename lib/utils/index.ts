...
export const getInitialChar = (fullName: string) => {
  return fullName.split(" ").map(word => word.charAt(0).toUpperCase()).slice(0, 2).join("")
}

export const getImageURL = (slug: string): string => `${drive}/lyrics/${slug}/image.webp`;
export const getBlurPlaceholderURL = (slug: string): string => `${drive}/lyrics/${slug}/blur-placeholder.webp`;
...
export const capitalizeText = (value: string) => {
  return value.includes(" ") ? value.split(" ").map(word => capitalize(word)).join(" ") : capitalize(value)
}
