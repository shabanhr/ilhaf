        ...
        <div className='w-full'>
            ...
            <div className="grid gap-3 md:grid-cols-2  lg:grid-cols-3 w-full">
                {reciters.map((reciter, i) => (
                    <Link href={reciter.link} key={i} className='hover:shadow-lg dark:hover:shadow-[#555] duration-200 flex items-center justify-start gap-2 border p-2 rounded-md'>
                        <Image
                            src={reciter.image}
                            width={60} height={60}
                            alt={reciter.name} className='rounded-full' />
                        {reciter.name}
                    </Link>
                ))}
            </div>
